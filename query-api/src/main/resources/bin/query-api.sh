...
do
	...
	stop)
	    STOP=true
	    QUERY_API_COMMAND="$
	    QUERY_API_COMMAND $var"
            shift
        ;;
        -h)
            echo "Usage: query-api.sh [command-options]"
        ...
        ;;
	*)
	    QUERY_API_COMMAND="$QUERY_API_COMMAND $var"
            shift
    esac
done
...
then
	...
	if $IS_DAEMON_MODE ; then
		...
		nohup $JAVA_HOME/bin/java -jar "$QUERY_API_HOME"/lib/query-api-0.1-SNAPSHOT.jar > ../query-api.out & echo $! > "../query-api-start_$!"
		cd ..
	else
        cd "$QUERY_API_HOME"/lib
