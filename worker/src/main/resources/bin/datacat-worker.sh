...
. `dirname $0`/setenv.sh
cd $DATACAT_WORKER_HOME

IS_DAEMON_MODE=false
DATACAT_WORKER_COMMAND=""
STOP=false
FORCE=false
...
do
	...
	stop)
        ...
        ;;
	*)
	    DATACAT_WORKER_COMMAND="$DATACAT_WORKER_COMMAND $var"
            shift
    esac
done
...
then
	...
	if $IS_DAEMON_MODE ; then
		...
		nohup $JAVA_HOME/bin/java -jar "$DATACAT_WORKER_HOME"/lib/worker-0.1-SNAPSHOT.jar > ../datacat-worker.out & echo $! > "../datacat-worker-start_$!"
		cd ..
	else
        cd "$DATACAT_WORKER_HOME"/lib
